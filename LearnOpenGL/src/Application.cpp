...
int main(void) {
  ...
  LearnOpenGL::Shader skybox_shader("../assets/shaders/4_6_skybox.vs",
                                    "../assets/shaders/4_6_skybox.fs");
  LearnOpenGL::Shader shader("../assets/shaders/4_6_reflection_map.vs",
                             "../assets/shaders/4_6_reflection_map.fs");
  LearnOpenGL::Shader framebuffer_shader(
      "../assets/shaders/4_5_framebuffer.vs",
      "../assets/shaders/4_5_framebuffer.fs");
  ...
  for (uint32_t i = 0; i < skybox_faces.size(); i++) {
    unsigned char* data =
        stbi_load(skybox_faces[i], &width, &height, &nrChannels, 0);
    if (data) {
      TY_CORE_INFO("Loaded cubemap face {}: {}x{}, {} channels", i, width,
                   height, nrChannels);
  ...
  // store all render data directly in buffer without conversions to
  // texture-specificc formats faster as writeable storage, but can not directly
  // read from them often used as depth and stencil attachments
  uint32_t rbo;
  ...
  // buffer, it is wise to use a renderbuffer object for that specific buffer.If
  // you need to sample data from a specific buffer like colors or depth values,
  // you should use a texture attachment

  // For a framebuffer to be complete the following requirements have to be
  // satisfied :
  ...
  // Bind the defualt framebuffer for rendering operations ot have a visual
  // impact
